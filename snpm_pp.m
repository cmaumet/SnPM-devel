...
if length(strmatch('MIPtable',Report))>0
  ...
  while max(STC_SnPMt.*(~bUsed))
    ...
    %-------------------------------------------------------------------
    nidm.PeakDefinitionCriteria_minDistanceBetweenPeaks = Dis;
    nidm.PeakDefinitionCriteria_maxNumberOfPeaksPerCluster = Num;
  ...
  nidm.Inferences(1).SearchSpaceMaskMap_searchVolumeInVoxels = S;
  nidm.Inferences(1).SearchSpaceMaskMap_searchVolumeInUnits = S*abs(prod(VOX));
  text(0,y,sprintf(TabDat.ftr{r,1},TabDat.ftr{r,2}), 'FontSize',8)
  y=y-0.8;  r = r + 1;
